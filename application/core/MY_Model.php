...
{        
	...
	function max_urut($table,$field=null,$where=null)
	{
		$this->db->select_max($field,'urut');
		if(!empty($where))
		{
			$this->db->where($where);
		}
	...
	function replace_batch($table,$data,$where)
	{
		if($table == FALSE || $data == FALSE || $where == FALSE)
		{
			return FALSE;
		...
		else
		{
			...
			{
				// $this->db->where($where);
				$update = $this->db->update_batch($table, $data, $where);
				return TRUE;
			}
		}
	}
	...
	public function delete_where_not_in($table,$where=null)
	{
		if(!empty($where))
		{
			$this->db->where_no_in($where);
		}
		if(!empty($table))
		{
			$this->db->delete($table); 

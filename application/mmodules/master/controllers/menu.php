...
class Menu extends MY_Controller {
    ...
    public function save_menu($value='')
    {
        ...
        if(is_numeric($id)){
            ...
            if($total <= 0){
                $save = $this->mm->write('menu',array('code'=>$param['code'],'nama'=>$param['nama'],'harga'=>$param['harga'],'hpp'=>$param['hpp'],'kategori'=>$param['kategori']));
                if($save==TRUE){
    ...
    public function add_mix_bahan()
    {
        ...
        if(is_numeric($id)){
            ...
            if($total <= 0){
                ...
                if($save==TRUE){
                    $save_blend = $this->mm->write('atombizz_blended_product',array('barcode_blended'=>$param['barcode_blended'],'barcode_product'=>$param['code'],'qty_product'=>$param['qty_product'],'status'=>$param['status'],'id_satuan'=>$param['id_satuan'],'category_bahan'=>$param['category_bahan']));
                    if($save_blend==TRUE){
                        echo '1|'.succ_msg('Berhasil menambahkan data komposisi.');
                    } else {
    ...
    public function detail_bahan_mix($value='')
    {
        $param = $this->input->post();
        $data = $this->mm->detail_bahan_mix($param['id']);
        echo json_encode($data);
    }
    ...
    public function save_mix_bahan()
    {
        ...
        $id = $param['ident'];
        if(is_numeric($id)){
            $replace = $this->mm->replace('atombizz_mix_product',array('qty_product'=>$param['qty_product'],'id_satuan'=>$param['id_satuan']),array('id'=>$id));
            if($replace){

...
class Mdl_menu extends MY_Model {
	...
	{
		$this->db->select('atombizz_mix_product.*, atombizz_product.name');
		$this->db->where('atombizz_mix_product.id',$value);
		$this->db->join('atombizz_product', 'atombizz_mix_product.barcode_product = atombizz_product.code', 'left');
		$query = $this->db->get('atombizz_mix_product');
		$result = $query->row();
		return $result;
	}

...
$this->thoni_fpdf->Cell(19,0.5,'DATA STOK GUDANG',0,0,'C');
...
$this->thoni_fpdf->SetWidths(array(1, 5, 6, 3.5, 3.5));
$this->thoni_fpdf->SetAligns(array('C','C','C','C','C'));
$this->thoni_fpdf->Row(array('No.', 'Kode Produk', 'Nama Produk', 'Nama Rak', 'Jumlah'));
$no = 1;
$i = 0;
$total = 0;
...
foreach ($stock as $key => $value) {
   ...
   $this->thoni_fpdf->Row(array($no, $value->product_code, $value->product_name, $value->rak_name, $value->saldo));
   // $total += $value->saldo;
   $no++;
}
...
// $this->thoni_fpdf->SetWidths(array(9, 3));
...
$this->thoni_fpdf->Output("laporan_stock.pdf","I");
?>
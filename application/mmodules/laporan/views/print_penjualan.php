...
$this->thoni_fpdf->SetFont('helvetica','B',12);
$this->thoni_fpdf->Cell(19,0.5,'DAFTAR PENJUALAN PERIODE '.tanggalIndo($setup['tgl_awal']).' s/d '.tanggalIndo($setup['tgl_akhir']),0,0,'C');
...
$this->thoni_fpdf->SetWidths(array(2, 5, 7, 5));
$this->thoni_fpdf->SetAligns(array('C','C','C','C'));
$this->thoni_fpdf->Row(array('No.', 'Kode Menu', 'Nama Menu', 'Qty'));
$no = 1;
$i = 0;
$total = 0;
...
foreach ($data as $key2 => $value2) {
   ...
   $this->thoni_fpdf->SetAligns(array('C','C','L','C'));
   $this->thoni_fpdf->Row(array($no, $value2->code, $value2->name, $value2->qty));
...
$this->thoni_fpdf->Output("laporan_penjualan.pdf","I");
?>
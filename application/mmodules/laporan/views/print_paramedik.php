...
$this->thoni_fpdf->Cell(16.5,1,'Atombizz',0,0,'R');
$this->thoni_fpdf->Ln(0.4);
$this->thoni_fpdf->SetFont('helvetica','',10);
...
$this->thoni_fpdf->Line(1,2.8,21,2.8);
$this->thoni_fpdf->Line(1,2.85,21,2.85);

$this->thoni_fpdf->Ln(1);
$this->thoni_fpdf->SetFont('helvetica','B',11);
$this->thoni_fpdf->Cell(21,0.5,'DATA PARAMEDIK',0,0,'C');
...
$this->thoni_fpdf->SetWidths(array(1, 6, 4,4, 4));
$this->thoni_fpdf->SetAligns(array('C','C','C','C','C'));
$this->thoni_fpdf->Row(array('No.', 'Invoice', 'Name', 'Tanggal', 'Total'));
$no = 1;
$i = 0;
$total = 0;
...
foreach ($param as $key => $value) {
   $this->thoni_fpdf->SetAligns(array('C','C','C','C','R'));
   $this->thoni_fpdf->Row(array($no, $value->invoice_no,$value->customer_name, tanggalIndo($value->date), format_rupiah($value->total)));
   $total += $value->total;
   $no++;
}
...
$this->thoni_fpdf->Output("laporan_paramedik.pdf","I");
?>
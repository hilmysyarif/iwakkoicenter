...
$this->thoni_fpdf->Cell(16,1,'Atombizz',0,0,'R');
$this->thoni_fpdf->Ln(0.4);
$this->thoni_fpdf->SetFont('helvetica','',10);
...
$this->thoni_fpdf->Line(1,2.8,17,2.8);
$this->thoni_fpdf->Line(1,2.85,17,2.85);

$this->thoni_fpdf->Ln(1);
$this->thoni_fpdf->SetFont('helvetica','B',11);
$this->thoni_fpdf->Cell(16,0.5,'DATA PIUTANG',0,0,'C');
...
$this->thoni_fpdf->SetWidths(array(1, 3.5, 4, 3.5, 4));
$this->thoni_fpdf->SetAligns(array('C','C','C','C', 'C','C'));
$this->thoni_fpdf->Row(array('No.', 'Kode Pelanggan', 'Nama Pelanggan', 'Jatuh Tempo','Saldo'));
$no = 1;
$i = 0;
$total = 0;
...
foreach ($stock as $key => $value) {
   $this->thoni_fpdf->SetAligns(array('C','C','L','C', 'C','R'));
   $this->thoni_fpdf->Row(array($no, $value->person, $value->name,tanggalIndo($value->jatuh_tempo), format_rupiah($value->saldo)));
   $total += $value->saldo;
   $no++;
}
...
$this->thoni_fpdf->SetWidths(array(12, 4));
...
$this->thoni_fpdf->Output("laporan_piutang.pdf","I");
?>
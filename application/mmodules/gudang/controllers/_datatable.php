...
class _datatable extends MY_Controller {
    ...
    {
        $this->datatables->select('id,date,reference_no,operator,status')
        ->edit_date('date','$1','date')
    ...
    public function list_inventaris($value='')
    {
        $this->datatables->select('id,category,product_code,name,in,out,saldo')
        ->from('view_inventaris_stok');

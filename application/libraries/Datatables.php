  ...
  */
  class Datatables
  {
    ...
    */
    ...
    protected $edit_currency  = array();
    protected $edit_date      = array();
    ...
    */
    ...
    /**
    * Sets additional column variables for editing column value to currency
    *
    * @param string $column
    * @param string $content
    * @param string $match_replacement
    * @return mixed
    */
    public function edit_currency($column, $content, $match_replacement)
    {
      $this->edit_currency[$column][] = array('content' => $content, 'replacement' => $this->explode(',', $match_replacement));
      return $this;
    }

    /**
    * Sets additional column variables for editing column value to indonesian date
    *
    * @param string $column
    * @param string $content
    * @param string $match_replacement
    * @return mixed
    */
    public function edit_date($column, $content, $match_replacement)
    {
      $this->edit_date[$column][] = array('content' => $content, 'replacement' => $this->explode(',', $match_replacement));
      return $this;
    }
    ...
    */
    ...
    protected function format_rupiah($value){
        $result = "Rp " . number_format((double)$value,2,',','.');
        return $result;
    }

    protected function TanggalIndo($date){
    ...
    protected function produce_output($charset)
    {
      ...
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        ...
        foreach($this->edit_columns as $modkey => $modval)
          ...
          // print_r($aaData[$row_key]);exit();
        foreach($this->edit_currency as $modkey => $modval)
          foreach($modval as $val)
            // print_r($val);exit();
            $aaData[$row_key][($this->check_mDataprop())? $modkey : array_search($modkey, $this->columns)] = $this->format_rupiah($this->exec_replace($val, $aaData[$row_key]));
          // print_r($aaData[$row_key]);exit();
        foreach($this->edit_date as $modkey => $modval)
          foreach($modval as $val)
            // print_r($val);exit();
            $aaData[$row_key][($this->check_mDataprop())? $modkey : array_search($modkey, $this->columns)] = $this->TanggalIndo($this->exec_replace($val, $aaData[$row_key]));
        //   // print_r($aaData[$row_key]);exit();

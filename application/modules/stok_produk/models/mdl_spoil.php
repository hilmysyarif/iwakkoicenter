...
class Mdl_spoil extends MY_Model {
	...
	public function _opt_produk($value='')
    {
    	...
    	foreach ($query->result() as $key => $value) {
    		$list .= '<option value="'.$value->code.'">'.$value->code.' - '.$value->nama.'</option>';
    	}
    ...
    {
    	$sql = "SELECT * FROM `kategori-menu`";
    	$query = $this->db->query($sql);
    	$list = '<option value=""></option>';
    	foreach ($query->result() as $key => $value) {
    		$list .= '<option value="'.$value->type.'">'.$value->nama.'</option>';
    	}
	...
	function get_list_produk($reference){
		$sql = "SELECT * FROM atombizz_spoil_tmp WHERE reference='$reference' AND status = 'produk' order by id asc";
	...
	function mencari($key){
        $sql = "SELECT * FROM menu WHERE code ='".$key['keyword']."'";
        $data = $this->db->query($sql);
        return $data;
    } 
	...
	{
		$sql = "SELECT * FROM menu WHERE `code`= ? ORDER BY id desc LIMIT 1";
		$data = $this->db->query($sql,array($code));
		foreach ($data->result_array() as $das) {
		}
		return (isset($das)) ? $das['harga'] : 0 ;
	}
	...
	function get_real_stok($param=''){
		$sql = "SELECT * FROM view_stok_produk WHERE code = ? AND status = 'produksi'";

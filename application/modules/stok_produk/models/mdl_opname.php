...
class Mdl_opname extends MY_Model {
	...
	function mencari($key){
        $sql = "SELECT * FROM view_stok_produk WHERE (code LIKE '%".$key."%' OR nama LIKE '%".$key."%') LIMIT 1";
    ...
    function detail($key){
        $sql = "SELECT * FROM menu WHERE code LIKE '%".$key."%' OR nama LIKE '%".$key."%' LIMIT 0,1";
        $data = $this->db->query($sql);
    ...
    function opt_keyword()
    {
    	...
    	$opt = '<option value="">Pilih Produk</option>';
    	foreach ($query->result() as $key => $value) {
    		$opt .= '<option value="'.$value->code.'">'.$value->code.' '.$value->nama.'</option>';
    	}
    	return $opt;
    }
...
/* Location: ./application/modules/stok_produk/models/mdl_opname.php */
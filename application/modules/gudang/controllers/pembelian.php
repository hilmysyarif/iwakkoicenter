...
class Pembelian extends MY_Controller {
    ...
    public function data()
    {
        ...
        $data['title']='Gudang - Pembelian Suplier';
    ...
    public function tambah_produk()
    {
        ...
        $data['title'] = 'Gudang - Pembelian Suplier';
        $data['opt_supplier'] = $this->mp->opt_supplier();
        $data['content'] = $this->load->view('/input_pembelian_produk',$data,TRUE);
        $this->load->view('template',$data);
    }
    ...
    public function get_total_biaya()
    {
     ...
     $get = $this->db->get_where('view_menu',array('id' => $id_produk));
     $hasil_get = $get->row();

     $harga_produk = $hasil_get->harga * $jumlah;
     echo 'Total : '.format_rupiah($harga_produk);

 }

 public function simpan_pembelian_produk() {
    ...
    if ($this->form_validation->run() == FALSE) {
        ...
        $get_produk = $this->db->get_where('view_stok_produk',array('id_product' => $id_produk));
        $hasil_get_produk = $get_produk->row();

        $data_produk['date'] = date('Y-m-d'); 
        $data_produk['status'] = 'pembelian produk';                
        $data_produk['rak_code'] = $hasil_get_produk->rak_code;
        $data_produk['in'] = $this->input->post("jumlah"); 
        $data_produk['out'] = 0;
        $data_produk['product_code'] = $id_produk;
        $this->db->insert("atombizz_warehouses_stok", $data_produk);


        //--------------------------------------------------------------------------------------

        $data_pembelian['reference_no'] = 'PP.'.$id_produk.'.'.date('Ymd');                
        $data_pembelian['note'] = date('his');
        $data_pembelian['date'] = date('Y-m-d'); 
        $data_pembelian['supplier_name'] = '-';
        $data_pembelian['subtotal'] = $hasil_get_produk->harga;
        $data_pembelian['total'] = $hasil_get_produk->harga * $this->input->post("jumlah");         
        $this->db->insert("atombizz_purchases", $data_pembelian);

        //-----------------------------------------------------------------------------------------
        $get_menu = $this->db->get_where('menu',array('id' => $id_produk));
        $hasil_get_menu = $get_menu->row();

        $data_item['reference_no'] = 'PP.'.$id_produk.'.'.date('Ymd');                
        $data_item['product_id'] = $this->input->post("produk");
        $data_item['product_code'] = $hasil_get_menu->code;
        $data_item['product_name'] = $hasil_get_menu->nama;
        $data_item['quantity'] = $this->input->post("jumlah");       
        $this->db->insert("atombizz_purchase_items", $data_item);
...
public function detail($id='')
{
    ...
    $data['title'] = 'Gudang - Detail Suplier';

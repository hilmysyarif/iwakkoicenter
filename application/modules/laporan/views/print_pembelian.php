...
$this->thoni_fpdf->Cell(24,1,'Atombizz',0,0,'R');
$this->thoni_fpdf->Ln(0.4);
$this->thoni_fpdf->SetFont('helvetica','',10);
...
$this->thoni_fpdf->Line(1,2.8,25,2.8);
$this->thoni_fpdf->Line(1,2.85,25,2.85);

$this->thoni_fpdf->Ln(1);
$this->thoni_fpdf->SetFont('helvetica','B',11);
$this->thoni_fpdf->Cell(25,0.5,'DAFTAR PEMBELIAN PERIODE '.tanggalIndo($setup['tgl_awal']).' s/d '.tanggalIndo($setup['tgl_akhir']),0,0,'C');
...
$this->thoni_fpdf->SetWidths(array(1, 3.5, 5, 3, 3, 1.5,2.5,4.5));
$this->thoni_fpdf->SetAligns(array('C','C','C','C','C','C','C','C'));
$this->thoni_fpdf->Row(array('No.', 'Tanggal', 'Referensi', 'Supplier', 'Nama Produk', 'Qty', 'Harga', 'Subtotal'));
$no = 1;
$i = 0;
$total = 0;
$this->thoni_fpdf->SetFont('helvetica', '', 9);
...
foreach ($data as $key2 => $value2) {
   ...
   $this->thoni_fpdf->SetAligns(array('C','L','L','C','C','C','R','R'));
   $this->thoni_fpdf->Row(array($no, tanggalIndo($value2->date), $value2->reference_no, $value2->supplier_name, $value2->product_name, $value2->quantity, format_rupiah($value2->unit_price), format_rupiah($value2->gross_total)));
   $total += $value2->gross_total;
   $no++;
}

// $diskon = ($value->discount == 0) ? format_rupiah($value->nominal_discount) : $value->discount;

$this->thoni_fpdf->SetWidths(array(19.5, 4.5));
...
$this->thoni_fpdf->Cell(28.5, 0.5, 'Created by : Atombizz.',0,'LR','L');

...
$this->thoni_fpdf->SetFont('helvetica','B',12);
$this->thoni_fpdf->Cell(19,0.5,'DAFTAR COMPLIMENT PERIODE '.tanggalIndo($setup['tgl_awal']).' s/d '.tanggalIndo($setup['tgl_akhir']),0,0,'C');
...
$this->thoni_fpdf->SetWidths(array(1, 3.5, 3, 3, 4.5, 4));
$this->thoni_fpdf->SetAligns(array('C','C','C','C','C','C'));
$this->thoni_fpdf->Row(array('No.','No Invoice', 'Tanggal', 'Meja', 'Jumlah', 'Oleh'));
$no = 1;
$i = 0;
$total = 0;
...
foreach ($data as $key => $value) {
   ...
   $this->thoni_fpdf->SetAligns(array('C','C','C','C','R','L'));
   $this->thoni_fpdf->Row(array($no, $value->code, tanggalIndo($value->date), $value->meja, format_rupiah($value->jumlah), $value->by));
   $total += $value->jumlah;
   $no++;
}
...
$this->thoni_fpdf->SetWidths(array(10.5, 4.5));
...
$this->thoni_fpdf->Output("laporan_compliment.pdf","I");
?>
...
$this->thoni_fpdf->SetFont('helvetica','B',12);
$this->thoni_fpdf->Cell(19,0.5,'DAFTAR OMSET PERIODE '.tanggalIndo($setup['tgl_awal']).' s/d '.tanggalIndo($setup['tgl_akhir']),0,0,'C');
...
$this->thoni_fpdf->Row(array('No.', 'Tanggal', 'Total Transaksi', 'Omset'));
$no = 1;
$i = 0;
$total = 0;
...
foreach ($omset as $key => $value) {
   ...
   $this->thoni_fpdf->SetAligns(array('C','C','C','R'));
   $this->thoni_fpdf->Row(array($no, tanggalIndo($value->tgl), $value->total_transaksi, format_rupiah($value->grand_total)));
...
$this->thoni_fpdf->SetWidths(array(12.5, 6.5));

      ...
      <div class="portlet box blue">
         <div class="portlet-title">
            ...
            <div class="actions">
               ...
               <a onclick="print('invoice')" class="btn green btn-sm"><i class="icon-print"></i> Cetak Bill</a>
         ...
         <div class="portlet-body form">
            ...
            <span id="flash_message"></span>
               <form id="form_data" enctype="multipart/form-data" method="post" >
                  ...
                  <!--<div class="col-md-3">
                     <select onchange="actCabang()" name="cabang" id="cabang" class="form-control select2me" data-placeholder="Pilih pembeli...">
                        <option selected value="Umum">Umum</option>
                        <option value="N1-003">N1-003</option>
                        <option value="N1-002">N1-002</option>
                     </select>
                  </div>-->
               </form>
            </div>
                  ...
                  <tr>
                     ...
                     <td>
                        <?php foreach ($_meja as $key => $value): ?>
                           <?php for ($i=1; $i <= $value->qty; $i++) { 
                              if (!in_array($value->code.'-'.$i, $_meja_isi)) {
                                 $is_checked = (in_array($value->code.'-'.$i, $_meja_tergabung)) ? 'checked' : '' ;
...
<script type="text/javascript">
   ...
   function actCabang()
   {
      var url = "<?php echo base_url($this->cname); ?>/change_cabang";
   ...
   function discount()
   {
      ...
      var tagihan = total - (total * discount / 100);
      var tax = tagihan / 10;
      tagihan += tax;
      $('#m-tax').val(tax);
      $('#m-tagih').val(tagihan);
   ...
   function print(Object) {
      ...
      $.ajax({
         ...
         success: function(msg)
         {
            ...
            sync_stock();
            if (reload) {
               window.location = "<?php echo base_url($this->cname); ?>/pesan/manufacture";
            }
         }
      });
   }
   ...
   function sync_stock(){
        ...
        var url = "<?php echo base_url();?>sync/get";
        $.ajax({
            async: false,
            type: "POST",
            url: url,
        ...
        var url = "http://mochi.cloud-astro.com/set.php";
        $.ajax({
            async: false,
            type: "POST",
            url: url,
        ...
        if (success) {
            var url = "<?php echo base_url();?>sync/success";
            $.ajax({
                type: "POST",
                url: url,
                data: {table:table},

...
class Mdl_kasir extends MY_Model {
	...
	public function take_stock($id='', $reference='', $operator='', $dept='')
	{
		$counter = 0;
		$data = array();
		$sql1 = $this->db->get_where('penjualan', array('id' => $id));
		$sql1 = $sql1->result();
		...
		foreach ($sql as $menu) {
			...
			foreach ($result as $key => $value) {
				$data[$counter] = array(
					...
					'in' => 0,
					'out' => $value->qty * $qty,
					'description' => 'Penjualan menu '.$value->menu_name.' dengan no referensi '.$reference.' kepada cabang '.$sql1[0]->cabang,
	...
	public function update_cabang($cabang='')
	{
		$sql = $this->db->query("UPDATE penjualan SET cabang = '$cabang' WHERE lunas = '0'");
		return $this->db->affected_rows();
	}
}